...
class NoiseFloorWidget(QtGui.QWidget):
    ...
    def __init__(self, driver):
        ...
        self.driver = driver      
        self.noise_floor = np.zeros(self.driver.n)
    ...
    def change_window(self, window):
        if window == 'Rectangular':
            ...
            self.driver.demod[0,:] = 0.49 * (1-np.cos(2*np.pi*np.arange(self.driver.n)/self.driver.n))
            self.driver.set_demod()
        elif window == 'Hamming':
            self.driver.demod[0,:] = 0.53836 * (0.46164-np.cos(2*np.pi*np.arange(self.driver.n)/self.driver.n))
            self.driver.set_demod()
        elif window == 'Blackman':
            self.driver.demod[0,:] = 0.42659-0.42659*np.cos(2*np.pi*np.arange(self.driver.n)/self.driver.n) + 0.076849*np.cos(4*np.pi*np.arange(self.driver.n)/self.driver.n)
            self.driver.set_demod()
